...
function calcular(){
    ...
    if(campo1.value != '' && campo2.value != ''){
        ...
        if(operacao==="dividir")
            ...
            resultado.classList.add("alert"); // validação: se não for digitado valores corretos nos dois campos. o campo resultado fica vermelho (feito no css)
            resultado.innerHTML = "Preencha os dois campos!"; // e aparece a mensagem de alerta.
            setTimeout(()=>{
                resultado.classList.remove("alert"); //essa função passada diretamente (função lambda/arrow function)
